...
const sms4freeConfig = config().sms4free || {};
  ...
  try {
    ...
    if (sms4freeApiKey && sms4freeUser && sms4freePass && sms4freeSender) {
      ...
      try {
        ...
        const smsResult: any = await response.json();
        console.log('ToriX SMS response:', smsResult);

        if (typeof smsResult?.status === 'number' && smsResult.status > 0) {
          res.status(200).json({
            success: true,
          ...
          const errText = smsResult?.message || 'Unknown SMS4FREE error';
          console.error('SMS4FREE SMS failed:', errText);
          throw new Error(errText);
        }
      } catch (sms4freeError: any) {
